...
class Bird:
    ...
    def update(self, key_lst: list[bool], screen: pg.Surface):
        ...
        if not (sum_mv[0] == 0 and sum_mv[1] == 0):  # 練習3：こうかとんの画像を向きを更新
...
class Beam:
    ...
    def __init__(self, bird: Bird):
        ...
        self.rct = self.img.get_rect()
        self.rct.centerx = bird.rct.centerx + bird.rct.width/1.5  # ビームの初期X座標をこうかとんの中心X座標から少し右に設定
        self.rct.centery = bird.rct.centery  # ビームの初期Y座標をこうかとんの中心Y座標に設定
        self.vx, self.vy = +5, 0
...
def main():
    ...
    explosions = []  # 複数の爆発エフェクトを扱うためのリストを生成
    ...
    while True:
        ...
        for beam in beams:
            ...
            for i, bomb in enumerate(bombs):
                if beam is not None and beam.rct.colliderect(bomb.rct):  # ビームと爆弾の衝突
                    explosions.append(Explosion(bomb.rct.center))  # Explosionインスタンスの生成
                    bombs[i] = None  # 爆弾を消滅
        ...
        for explosion in explosions:
            explosion.update(screen)  # 爆発エフェクトの表示
